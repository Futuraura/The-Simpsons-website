...
function renderNotFound() {
  contentElement.innerHTML =
    '<h1 class="error404">404 Not Found</h1><p class="error404-description">The requested character does not exist.</p>';
}
...
function populateCharacterContent() {
  fetch("assets/data/characters.json")
    ...
    .then((characters) => {
      const characterName = new URL(window.location.href).searchParams.get(
        "character"
      );
      ...
      character.voiceClips.forEach((clip) => {
        ...
        clipElement.innerHTML = `
                ...
                <audio controls class="voice-clip">
                    <source src="${clip.source}" type="${getAudioType(
          clip.source
        )}">
                </audio>
            `;
